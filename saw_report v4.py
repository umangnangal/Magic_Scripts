...
def sa_log_generation():
    ...
    if len(sys.argv) == 1:
        ...
        #Creating a directory for storing the log generated by coverity for each component
...
def sa_warning_analysis(file):
    ...
    #Iterating through the warnings to get the count of each warning type
    for key in warning_record.keys():
        for warning_list in warning_record[key].values():
            for item in warning_list:
...
For per-file SA warnings count report, please uncomment '#File-based report generation' section of the code.''')
