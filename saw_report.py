...
def sa_log_generation():
    ...
    if len(sys.argv) == 1:
        ...
        print('''Correct usage: ./saw_report -comp_filelist <comp_list> [-sub_ws <sub_workspace>] -branch <branch_name> [-noprompt]
                                ./saw_report -comp <comp_name> [-sub_ws <sub_workspace>] -branch <branch_name> [-noprompt]
                NOTE : Component name should be relative to the root directory of the pulled LBT or to the sub_ws if specified.''')
...
\nSpecify the sub_ws(in case of N7K/MDS platform) and branch using -sub_ws < > and -branch < > respectively.
        Eg. ./saw_report -comp infra/port_manager/fc-pm -branch iplus_dev
            ./saw_report -comp_filelist file.txt -sub_ws N7KMDS -branch skywalker
\nNOTE : Component name should be relative to the root directory of the pulled LBT or to the sub_ws if specified.''')

    elif len(sys.argv) >= 3 and sys.argv[1] in ['-comp', '-comp_filelist'] :
        ...
        print('\nPerforming SA Warning Analysis for following modules:')
        for module in module_list:
            if module in N7KMDS_lineup:
                exists = "component exists in N7KMDS"
            elif module in COMMON_lineup:
                exists = "component exists in COMMON"
            elif module in ROOT_lineup:
                exists = "component exists in ROOT"
            else:
                exists = "Component does not exist in any of the component lineup files. Coverity might not run for this conmponent."
            print('{0} -------------- {1}'.format(module, exists))
        ...
        for module in module_list:
            ...
            if len(sys.argv)>=7 :
                sub_ws = sys.argv[4]
                branch = sys.argv[6]
                cmd1 = '/auto/andatc/independent/acme_scripts/1.0/bin/static_dcos -acme_comp {0} -sub_ws {3} -branch {4} > ./{1}/{2} 2>&1 < /dev/null'.format(module, count_dir, module.replace('/','.'), sub_ws, branch)
            elif len(sys.argv) >= 5:
                branch = sys.argv[4]
