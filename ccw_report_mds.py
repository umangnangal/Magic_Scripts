...
def warning_analyze_print(name = ''):
    ...
    for file in glob.glob('*'):
        ...
        for div, div_count in warning_count.items():
            if div_count>0:
                print(div + '-'*(50-len(div)) + str(div_count))
        if len(others)>0:
...
def ccw_report():
    ...
    if len(sys.argv) == 1:
        ...
        module_path_file2 = './defs/override_modulepath_n7k.mk'
        module_path2name = {}
        ...
        f = io.open(module_path_file2, 'r', encoding='latin1')
        for line in f:
            if ':=' in line:
                line = line.strip().partition(':=')
                module_path2name[line[2].strip()] = line[0].strip().rpartition('_')[0]
        ...
        for log_file in log_files:
            ...
            try:
                ...
                print('{0} - Logfile not present'.format(log_file))
                os.chdir('./..')
            else:
                for line in f:
                    line = line.strip()
                    if 'warning' in line and '.mk' not in line and '.c' in line and '.cli' not in line and '.cmd' not in line:
                        ...
                        while(loop>0):
                            ...
                            if path in module_path2name.keys():
                                ...
                                loop -= 1
                        if flag == 0 :
                            line = warning.replace('//', '/')
                            line = line.partition('../')
                            line = line[1] + line[2]
                            line = [ x for x in line.partition(':')[0].split('/') if '.c' not in x and 'N7KMDS' not in x]
                            loop = len(line)
                            while(loop>0):
                                path = '/'.join(line[:loop]).strip()
                                if path in module_path2name.keys():
